...
pkg.isTouchable = pkg.isInBrowser && (!!('ontouchstart' in window ) || !!('onmsgesturechange' in window));
...
pkg.findInTree = function(root, path, eq, cb) {
    ...
    while (m = findRE.exec(path)) {
        if (m[2] == null || m[2].trim().length === 0) {
            throw new Error("Empty path name element");
        }
        ...
        this.mergeContent = function(o, v) {
            ...
            if (Array.isArray(v)) {
                if (o && Array.isArray(o) === false) {
                    throw new Error("Array merging type inconsistency");
                }
...
var ListModelListeners = MB.Listeners.Class("elementInserted", "elementRemoved", "elementSet");
pkg.ListModel = Class([
    function() { this.$this([]); },

    function(a) {
        this._ = new ListModelListeners();
        this.d = a;
    },
...
var TreeModelListeners = MB.Listeners.Class("itemModified", "itemRemoved", "itemInserted");
pkg.TreeModel = Class([
    function $clazz() {
    ...
    function(r) {
        ...
        this._ = new TreeModelListeners();
    }
]);

var MatrixListeners = MB.Listeners.Class("matrixResized", "cellModified");
pkg.Matrix = Class([
    function $prototype() {
...
zebra.ready(function() {
    ...
    try {
        ...
        pkg.$configuration.loadByUrl("canvas.json", pkg);
        var p = zebra()['canvas.json'];
        if (p) pkg.$configuration.loadByUrl(p, pkg);
...
var MB = zebra.util,Composite = pkg.Composite, ME = pkg.MouseEvent, MouseListener = pkg.MouseListener,
    ...
    timer = zebra.util.timer, KeyListener = pkg.KeyListener, Cursorable = pkg.Cursorable,
    ChildrenListener = pkg.ChildrenListener,
...
pkg.MouseWheelSupport = Class([
    ...
    function setup(canvas) {
        ...
        var $this = this;
        canvas.canvas.addEventListener ("mousewheel", function(e) { $this.mouseWheelMoved(e); }, false);
        canvas.canvas.addEventListener ("DOMMouseScroll", function(e) { $this.mouseWheelMoved(e); }, false);
    }
]);
...
pkg.StatePan = Class(pkg.ViewPan, Composite, MouseListener, KeyListener, [
    ...
    function $prototype() {
        ...
        this.addFocusComponent = function(c) {
            if (this.focusComponent != c) {
                if (c != null && this.kids.indexOf(c) >= 0) {
                    throw Error("Focus componet already exists");
                }
...
pkg.Extender = Class(pkg.Panel, MouseListener, Composite, [
    function $prototype() {
        this.catchInput = function(child){ 
            return child != this.contentPan && !L.isAncestorOf(this.contentPan, child); 
        };
    },
...
pkg.Scroll = Class(pkg.Panel, MouseListener, zebra.util.Position.PositionMetric, Composite, [
    function $clazz() {
    ...
    function $prototype() {
        ...
        this.mouseDragStarted = function (e){
           //!!! It is more conviniet to  if (this.isDragable || this.isInBundle(e.x, e.y)){
    ...
    function $prototype() {
        ...
        this.mouseDragged = function (e){
            if (e.mask == ME.LEFT_BUTTON){
                var p = this.getTextRowColAt(this.view, e.x - this.scrollManager.getSX(), e.y - this.scrollManager.getSY());
                if (p != null) this.position.setRowCol(p[0], p[1]);
            }
        };
...
pkg.CompList = Class(pkg.BaseList, pkg.Composite, [
    ...
    function setPosition(c){
        if (c != this.position){
            if (zebra.instanceOf(this.layout, Position.PositionMetric)) {
                c.setPositionMetric(this.layout);
            }
            this.$super(c);
        }
    },
...
pkg.showWindow = function(context, type, win) {
    if (arguments.length < 3) {
        win = type;
        type = pkg.WinLayer.INFO;
    }
    var canvas = pkg.findCanvas(context);
    return canvas.getLayer("win").addWin(type, win, null);
};
...
pkg.WinLayer = Class(pkg.BaseLayer, pkg.ChildrenListener, [
    ...
    function $prototype() {
        ...
        this.activate = function(c){
            if (c != null && (this.winsInfo.hasOwnProperty(c) === false ||
                              this.winType(c) == pkg.WinLayer.INFO))
            {
                throw new Error();
            }
    ...
    function insert(index, constr, lw) {
        ...
        if (info[1] != pkg.WinLayer.MDI && info[1] != pkg.WinLayer.MODAL && info[1] != pkg.WinLayer.INFO) {
            throw new Error();
        }
...
var L = zebra.layout, Cursor = ui.Cursor, KeyEvent = ui.KeyEvent, Cursorable = ui.Cursorable, CURSORS = [];
...
pkg.ShaperPan = Class(ui.Panel, Cursorable, ui.Composite, ui.KeyListener, ui.MouseListener, [
    function $prototype() {
        ...
        this.keyPressed = function(e) {
            if (this.kids.length > 0){
                ...
                if (b) {
                    if (this.isResizeEnabled && w > this.shaperBr.gap * 2 && h > this.shaperBr.gap * 2) this.setSize(w, h);
                }
                else {
